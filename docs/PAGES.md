...
| 기능 ID | 기능명 | 설명 | 상태 |
|--------|-------|------|------|
| P1-001 | 게시물 작성 | 제목, 본문, 카테고리, 태그를 포함한 새 게시물 생성 | 🔴 대기 |
| P1-002 | 게시물 조회 | 게시물 ID로 단일 게시물 상세 정보 조회 | 🔴 대기 |
| P1-003 | 게시물 수정 | 기존 게시물의 제목, 본문 등 수정 | 🔴 대기 |
| P1-004 | 게시물 삭제 | 게시물 삭제 (소프트 삭제 지원) | 🔴 대기 |
| P1-005 | 게시물 목록 조회 | 페이징, 정렬, 필터링이 적용된 목록 조회 | 🔴 대기 |
| P1-006 | 조회수 관리 | 게시물 조회 시 조회수 자동 증가, 중복 방지 | 🔴 대기 |
| P1-007 | 상단고정 설정 | 특정 게시물을 목록 상단에 고정 | 🔴 대기 |
| P1-008 | 상단고정 해제 | 상단 고정된 게시물의 고정 해제 | 🔴 대기 |
| P1-009 | 임시저장 | 작성 중인 게시물 임시 저장 | 🔴 대기 |
| P1-010 | 임시저장 목록 | 임시 저장된 게시물 목록 조회 | 🔴 대기 |
...
| 기능 ID | 기능명 | 설명 | 상태 |
|--------|-------|------|------|
| P2-001 | 댓글 작성 | 게시물에 댓글 작성 | 🔴 대기 |
| P2-002 | 댓글 조회 | 게시물의 댓글 목록 조회 | 🔴 대기 |
| P2-003 | 댓글 수정 | 본인 댓글 수정 | 🔴 대기 |
| P2-004 | 댓글 삭제 | 본인 댓글 삭제 | 🔴 대기 |
| P2-005 | 대댓글 작성 | 댓글에 대한 답글 작성 | 🔴 대기 |
| P2-006 | 대댓글 조회 | 특정 댓글의 대댓글 목록 조회 | 🔴 대기 |
| P2-007 | 게시물 좋아요 | 게시물에 좋아요 추가 | 🔴 대기 |
| P2-008 | 게시물 좋아요 취소 | 게시물 좋아요 취소 | 🔴 대기 |
| P2-009 | 댓글 좋아요 | 댓글에 좋아요 추가 | 🔴 대기 |
| P2-010 | 북마크 추가 | 게시물 북마크 | 🔴 대기 |
| P2-011 | 북마크 해제 | 게시물 북마크 해제 | 🔴 대기 |
| P2-012 | 북마크 목록 | 사용자의 북마크 목록 조회 | 🔴 대기 |
...
| 기능 ID | 기능명 | 설명 | 상태 |
|--------|-------|------|------|
| P3-001 | 파일 업로드 | 단일/다중 파일 업로드 | 🔴 대기 |
| P3-002 | 이미지 업로드 | 이미지 파일 업로드 및 미리보기 | 🔴 대기 |
| P3-003 | 썸네일 생성 | 이미지 업로드 시 자동 썸네일 생성 | 🔴 대기 |
| P3-004 | 파일 다운로드 | 첨부 파일 다운로드 | 🔴 대기 |
| P3-005 | 파일 삭제 | 첨부 파일 삭제 | 🔴 대기 |
| P3-006 | 업로드 제한 | 파일 크기, 확장자, 개수 제한 | 🔴 대기 |
| P3-007 | 기본 검색 | 제목, 본문 키워드 검색 | 🔴 대기 |
| P3-008 | 태그 검색 | 태그 기반 검색 | 🔴 대기 |
| P3-009 | 작성자 검색 | 작성자명 기반 검색 | 🔴 대기 |
| P3-010 | 복합 검색 | 여러 조건 조합 검색 | 🔴 대기 |
...
- [ ] 모든 API 엔드포인트 구현 완료
- [ ] 모든 테스트 케이스 통과 (15개)
- [ ] 파일 검증 로직 구현 (크기, 확장자, 시그니처)
- [ ] 썸네일 자동 생성 구현
- [ ] 전문 검색 지원 (SQL LIKE 또는 Full-Text Search)
...
| 기능 ID | 기능명 | 설명 | 상태 |
|--------|-------|------|------|
| P4-001 | 전체 게시물 관리 | 관리자용 전체 게시물 조회/관리 | 🔴 대기 |
| P4-002 | 전체 댓글 관리 | 관리자용 전체 댓글 조회/관리 | 🔴 대기 |
| P4-003 | 신고 목록 조회 | 신고된 콘텐츠 목록 조회 | 🔴 대기 |
| P4-004 | 신고 처리 | 신고 승인/거부 처리 | 🔴 대기 |
| P4-005 | 콘텐츠 블라인드 | 부적절한 콘텐츠 숨김 처리 | 🔴 대기 |
| P4-006 | 일괄 삭제 | 선택된 게시물/댓글 일괄 삭제 | 🔴 대기 |
| P4-007 | 통계 조회 | 게시판 통계 데이터 조회 | 🔴 대기 |
| P4-008 | 질문 작성 | Q&A 질문 작성 | 🔴 대기 |
| P4-009 | 질문 조회 | Q&A 질문 상세/목록 조회 | 🔴 대기 |
| P4-010 | 답변 작성 | Q&A 답변 작성 | 🔴 대기 |
| P4-011 | 답변 채택 | 질문자가 답변 채택 | 🔴 대기 |
| P4-012 | 답변 추천 | 답변 추천/비추천 | 🔴 대기 |
